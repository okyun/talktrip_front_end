...
import { getProductClickStatsRange } from '../../../common/api/adminApi';
...
import {
  buildProductClickExportRows,
  exportProductClickStatsExcel,
  exportProductClickStatsPdf,
  formatRangeLabel,
  formatEventTime,
...
const AdminProductClickStatsPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await getProductClickStatsRange({
        startTimeMs: start,
        endTimeMs: end,
        limit: LIMIT,
      });
  ...
  const sortedData = useMemo(() => {
    ...
    return [...items]
      .sort((a, b) => {
        const c = Number(b?.clickCount || 0) - Number(a?.clickCount || 0);
        if (c !== 0) return c;
  ...
  const handleExcelDownload = async () => {
    ...
    try {
      ...
      await yieldToPaint();
      const rows = buildProductClickExportRows(sortedData, productNameById);
      exportProductClickStatsExcel(rows, exportMeta);
    } catch (e) {
      console.error(e);
  ...
  const handlePdfDownload = async () => {
    ...
    try {
      ...
      await yieldToPaint();
      await exportProductClickStatsPdf(tableRef.current, exportMeta);
    } catch (e) {
      console.error(e);
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Redis ZINCRBY 합산 /api/trending/products/clicks/range</p>
      ...
      {!loading && !error && sortedData.length > 0 && (
        <div ref={tableRef} className="bg-white shadow rounded-lg p-4">
          <h2 className="text-lg font-semibold mb-1">클릭 상위 상품</h2>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left">클릭 시간</th>
                </tr>
              </thead>
              <tbody>
                {sortedData.map((item, idx) => (
                  <tr key={`${item.productId ?? 'unknown'}-${idx}`} className="border-b last:border-0">
                    ...
                    <td className="px-3 py-2 text-right text-gray-800">
                      {Number(item.clickCount || 0).toLocaleString()}
                    </td>
                    <td className="px-3 py-2 text-gray-700 text-xs">{formatEventTime(item.clickedAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
